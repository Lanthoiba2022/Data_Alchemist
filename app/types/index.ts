...
export const ClientSchema = z.object({
  ...
  RequestedTaskIDs: z.string().transform((str) => 
    str.split(',').map(id => id.trim()).filter(id => id.length > 0)
  ),
  GroupTag: z.string().optional(),
  AttributesJSON: z.string().transform((str) => {
    try {
...
export const WorkerSchema = z.object({
  ...
  AvailableSlots: z.string().transform((str) => {
    try {
      return JSON.parse(str);
    } catch {
      return [];
...
export const TaskSchema = z.object({
  ...
  RequiredSkills: z.string().transform((str) => 
    str.split(',').map(skill => skill.trim()).filter(skill => skill.length > 0)
  ),
  PreferredPhase: z.string().transform((str) => {
    try {
      return JSON.parse(str);
    } catch {
      return [];
